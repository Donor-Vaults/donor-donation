...
const options = [
  {
    label: (
      <span style={{ color: "#02A95C", display: "flex", alignItems: "center" }}>
        <svg
          width="37"
          height="25"
          viewBox="0 0 37 40"
        ...
        <Mobile style={{ fontSize: "1.6rem", margin: "0 0 0 0.5rem" }}>
          About
        </Mobile>
      </span>
    ),
...
const DocsTab = ({fundraiser}) => {
  ...
  <div
    ...
    <div>
      {fundraiser.supportingDocuments.map((e) => {
      

        return <object width="100%" height="400" data={e} type="application/pdf">   </object>
...
const BigCard = ({ fundraiser }) => {
  ...
  const renderTab = () => {
    if (selectedTabIndex == 0) {
      ...
      return <UpdateTab fundraiser={ fundraiser} />
    }else  if (selectedTabIndex == 2) {
      return <DocsTab fundraiser={ fundraiser} />
    }
  }
